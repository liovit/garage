...
<title>{{ __('Garage | Delete Order') }} {{ $order->id }}</title>
        ...
        <div id="kt_content_container" class="container-xxl pb-6">
            ...
            <div class="alert alert-dismissible bg-light-danger d-flex flex-column flex-sm-row p-5 mb-10">
                ...
                <div class="d-flex flex-column pe-0 pe-sm-10">
                    ...
                    <!--end::Title-->
                    <!--begin::Content-->
                    {{ __('Are you sure you want to permanently delete this work order?') }}
                    <!--end::Content-->
                </div>
                <!--end::Wrapper-->
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <table class="table table-bordered table-striped" style="margin-bottom: 0;">
                                <tr>
                                    <th style="">{{ __('Order ID') }}</th>
                                    <td field-key='name'>{{ $order->id }} <a href="{{ url('/work/orders/'.$order->id) }}" class="btn btn-sm btn-secondary" target="_blank"  style="float:right;">{{ __('Open in a new tab') }}</a></td>
                                </tr>
                                <tr>
                                    <th>{{ __('Customer') }}</th>
                                    <td field-key='email'>{{ $customer->name }} || {{ $customer->company }} <a href="{{ url('/management/customers/'.$customer->id) }}" class="btn btn-sm btn-secondary" target="_blank" style="float:right;">{{ __('Open in a new tab') }}</a></td>
                                </tr>
                                <tr>
                                    <th>{{ __('Vehicle') }}</th>
                                    @if($vehicle)
                                        <td>{{ $vehicle->make }} {{ $vehicle->model }} || {{ $vehicle->vin_code }} <a href="{{ url('/work/vehicles/'.$vehicle->id) }}" class="btn btn-sm btn-secondary" target="_blank"  style="float:right;">{{ __('Open in a new tab') }}</a></td>
                                    @else 
                                        <td> - </td>
                                    @endif
                                </tr>
            ...
            <div class="row">
                <div class="col-6 form-group">
                    <form action="{{ url('/work/orders/delete/'.$order->id) }}" method="post">
                        @csrf
                        <button type="submit" class="col-12 btn btn-success"><i class="fas fa-check-circle"></i> {{ __('Yes') }}</button>
                    </form>
                </div>
                <div class="col-6 form-group">
                    <a href="{{ url('/work/orders') }}" class="col-12 btn btn-danger"><i class="fas fa-times-circle"></i> {{ __('No') }}</a>
                </div>
            </div>
